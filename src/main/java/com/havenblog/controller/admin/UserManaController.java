package com.havenblog.controller.admin;
...
public class UserManaController {
    ...
    public RespModel updateUserEnabled(Boolean enabled, Long uid) {
        if (userService.updateUserEnabled(enabled, uid) == 1) {
            return new RespModel("success", "更新成功!");
        } else {
            return new RespModel("error", "更新失败!");
        }
    }

    @RequestMapping(value = "/user/{uid}", method = RequestMethod.DELETE)
    public RespModel deleteUserById(@PathVariable Long uid) {
    ...
    public RespModel updateUserRoles(Long[] rids, Long id) {
